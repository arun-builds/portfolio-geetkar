...
export default function ProfileButton({ platform }: { platform: string }) {
    ...
    return (
        <div className=" ">
            ...
            <div className={`${platform === "other" ? "bg-zinc-800 text-gray-100 p-2 border hover:bg-zinc-800/80 transition-all" : "rounded-t-none"} w-full ${isEditMode && platform !== "other" ? "flex items-center justify-between" : ""} border flex items-center justify-center gap-2 bg-white  text-black p-1 rounded-md hover:cursor-pointer hover:bg-gray-200/100 transition-all`}>
